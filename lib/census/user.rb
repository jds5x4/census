module Census
  module User
    ...
    module Scopes
      ...
      def self.included(model)
        model.class_eval do
          scope :with_matching_census_answers, lambda { |user, limit|
            {
    ...
    module InstanceMethods
      ...
      #
      def census_data
        @census_data ||= UserData.new(self)
      end

      def unanswered_questions_for(data_group)
        questions = data_group.questions
        questions.select {|q| first_answer_for(q).data.blank?}
      end
      ...
      def has_answered_all_questions_for?(data_group)
        questions = data_group.questions
        questions.each {|q| return false if first_answer_for(q).data.blank?}
        true
      end
